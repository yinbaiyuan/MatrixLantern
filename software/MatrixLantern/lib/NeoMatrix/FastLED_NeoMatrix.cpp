...
void FastLED_NeoMatrix::drawPixel(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b)
{

  if((x < 0) || (y < 0) || (x >= _width) || (y >= _height)) return;

  _leds[XY(x,y)] = passThruFlag ? passThruColor : expandColor(FastLED_NeoMatrix::Color(r,g,b));
}
